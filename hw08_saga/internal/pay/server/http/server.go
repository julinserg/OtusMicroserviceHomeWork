...
type Storage interface {
	RegisterUser(user pay_app.UserAuth) (int, error)
	GetUser(login string, password string) (pay_app.UserAuth, error)
}
...
func NewServer(logger Logger, storage Storage, endpoint string) *Server {
	...
	sessions := make(map[string]pay_app.UserAuth)
